...
interface AuthContextValue { user: User | null; token: string | null; loading: boolean; login: (email: string, password: string) => Promise<void>; register: (name: string, email: string, password: string) => Promise<void>; logout: () => void; }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (name: string, email: string, password: string) => { const result = await api.register(name, email, password); storage.setToken(result.accessToken); storage.clearOrganizationId(); setToken(result.accessToken); setUser(result.user); };
